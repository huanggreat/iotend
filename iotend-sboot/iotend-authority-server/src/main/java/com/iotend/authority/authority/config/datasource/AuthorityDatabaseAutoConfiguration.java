...
/**
 * iotend.database.multiTenantType != DATASOURCE 时，该类启用.
 * 此时，项目的多租户模式切换成：${iotend.database.multiTenantType}。
 * <p>
 * NONE("非租户模式"): 不存在租户的概念
 ...
 * @author huang
 * 断点查看原理：👇👇👇
 * @see com.iotend.database.datasource.BaseMybatisConfiguration#mybatisPlusInterceptor()
 * @see com.iotend.database.servlet.TenantContextHandlerInterceptor
 * @see com.iotend.database.parsers.DynamicTableNameParser
 */
@Configuration
@Slf4j
@MapperScan(
        basePackages = {"com.iotend",},
        annotationClass = Repository.class,
        sqlSessionFactoryRef = AuthorityDatabaseAutoConfiguration.DATABASE_PREFIX + "SqlSessionFactory")
@EnableConfigurationProperties({MybatisPlusProperties.class})
@ConditionalOnExpression("!'DATASOURCE'.equals('${iotend.database.multiTenantType}')")
public class AuthorityDatabaseAutoConfiguration extends BaseDatabaseConfiguration {
    ...
    public AuthorityDatabaseAutoConfiguration(MybatisPlusProperties properties,
        ...
        log.debug("检测到 iotend.database.multiTenantType!=DATASOURCE，加载了 AuthorityDatabaseAutoConfiguration");
    }
