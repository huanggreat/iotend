package com.iotend.authority.interceptor;
...
import static com.iotend.context.BaseContextConstants.*;
import static com.iotend.exception.code.ExceptionCode.JWT_OFFLINE;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            if (isDev() && (StrPool.TEST_TOKEN.equalsIgnoreCase(token) || StrPool.TEST.equalsIgnoreCase(token))) {
                authInfo = new AuthInfo().setAccount("huang").setUserId(3L).setTokenType(BEARER_HEADER_KEY).setName("平台管理员");
            }
            // 4, 解析 并 验证 token
            if (authInfo == null) {
