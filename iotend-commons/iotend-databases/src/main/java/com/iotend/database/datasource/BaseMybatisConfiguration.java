package com.iotend.database.datasource;
...
import com.iotend.database.injector.MySqlInjector;
import com.iotend.database.mybatis.WriteInterceptor;
import com.iotend.database.mybatis.typehandler.FullLikeTypeHandler;
import com.iotend.database.mybatis.typehandler.LeftLikeTypeHandler;
import com.iotend.database.mybatis.typehandler.RightLikeTypeHandler;
import com.iotend.database.plugins.SchemaInterceptor;
import com.iotend.database.properties.DatabaseProperties;
import com.iotend.database.properties.MultiTenantType;
import com.iotend.database.servlet.TenantWebMvcConfigurer;
import com.iotend.uid.service.DisposableWorkerIdAssigner;
...
/**
 * Mybatis 常用重用拦截器，iotend.database.multiTenantType=任意模式 都需要实例出来
 * <p>
 * 拦截器执行一定是：
...
public abstract class BaseMybatisConfiguration {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        ...
        if (MultiTenantType.SCHEMA.eq(this.databaseProperties.getMultiTenantType())) {
            ...
            log.info("检测到 iotend.database.multiTenantType=SCHEMA，已启用 SCHEMA模式");
        } else if (MultiTenantType.COLUMN.eq(this.databaseProperties.getMultiTenantType())) {
            log.info("检测到 iotend.database.multiTenantType=COLUMN，已启用 字段模式");
            // COLUMN 模式 多租户插件
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnExpression("'DEFAULT'.equals('${iotend.database.id-type:DEFAULT}') || 'CACHE'.equals('${iotend.database.id-type:DEFAULT}')")
